...
def adjust_predicts(score, label,
    ...
    if len(score) != len(label):
        print(score.shape[0],label.shape[0])
...
def calc_seq(score, label, threshold, calc_latency=False, model='adj'):
    """
...
def pot_eval(init_score, score, label, q=1e-5, level=0.02,dynamic=True):
    """
    ...
    ret = s.run(dynamic=dynamic, with_alarm=False)  # run
    # print(len(ret['alarms']))
    # print(len(ret['thresholds']))
    pot_th = np.mean(ret['thresholds']) * lm[1]
    # pot_th = np.percentile(score, 100 * lm[0])
    # np.percentile(score, 100 * lm[0])
    pred, p_latency = adjust_predicts(score, label, pot_th, calc_latency=True, model='nom')
