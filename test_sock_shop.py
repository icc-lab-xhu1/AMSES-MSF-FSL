...
def main(config):
    ...
    if config.mode == "pretrain":
        ...
        result_rca = {}
        print("======================PRE TRAIN ======================")
        data_name = ['cpu', 'mem', 'latency']
        for name in data_name:
            wps, lstm, mhar, mad, usad, dag, mtad, omni, cae, msc = [], [], [], [], [], [], [], [], [], []
            # rca_pc, rca_ges, rca_lin = [], [], []
            # wps, mhar, mad, usad, dag, mtad, omni, cae = [], [], [], [], [], [], [], []
            pretrain = Pretrain(vars(config))
            for m in range(5):
                epoch = 10
                # train, test_cpu, pre_label, input_cpu, values_1, rca_data = chouyang(name)
                train, test_cpu, pre_label, input_cpu, values_1 = chouyang(name, sample_rca=False)
                i = len(train)/6
                low = i * m
                high = i * (m+1)
                # random_train = random.randint(0, len(train) - 500)
                random_train = random.randint(int(low), int(high))
                print(random_train)
                train = train[random_train: random_train + 500, :]
                ...
                # lstm.append([v for k, v in dict_cpu.items() if k == 'LSTM-VAE'])
            ...
            # dict = {'WPS': wps, 'LSTM-VAE': lstm, 'MHAR': mhar, 'MAD_GAN': mad, 'USAD': usad, 'DAGMM': dag, 'MTAD': mtad, 'Omni': omni, 'CAE_M': cae, 'MSCRED': msc}
        ...
        print(result)
        with open(r'./result/'+config.dataset+'_result_sock', 'wb') as f:
            pk.dump(result, f)
    ...
    if config.mode == 'train':
        ...
        with open(r'./result/' + config.dataset + '_result_sock', 'rb+') as f:
            result = pk.load(f)
        f.close()
        ...
        for i in range(len(result)):
            ...
            with open(r'./thred/' + config.dataset + '_' + data_name + '.pkl', 'wb') as f:
                pk.dump(thred, f)
            print('thred:', thred)
        ...
        C = metrics.confusion_matrix(data['label'].values, data['class'].values,
                                     labels=[0, 1, 2, 3])  # 可将'1'等替换成自己的类别，如'cat'。
        ...
        plt.xticks(range(0, 4), labels=['Normal', 'CPU', 'Memory', 'Latency'])  # 将x轴或y轴坐标，刻度 替换为文字/字符
        plt.yticks(range(0, 4), labels=['Normal', 'CPU', 'Memory', 'Latency'])
        plt.savefig(r'./figure/' + config.dataset + '.pdf', dpi=200, bbox_inches='tight')
        ...
        print('宏平均F1-score:', metrics.f1_score(data['label'].values, data['class'].values, labels=[0, 1, 2, 3], average='macro'))  # 预测宏平均f1-score输出
        ...
        print('微平均F1-score:', metrics.f1_score(data['label'].values, data['class'].values, labels=[0, 1, 2, 3],average='micro'))  # 预测微平均f1-score输出
        ...
        for name in data_name:
            ...
            for m in range(10):
                ...
                rca_data = chouyang(name, sample_rca = True)
                ...
                for r in rca_data.columns:
                    if r.split('_')[-1] != 'latency':
                        data_rca = data_rca.drop(r, axis=1)

                col = list(data_rca.columns)
                true_root_cause = [col.index(config.dataset + '_ctn_latency')]
        ...
        with open(r'./result/' + config.dataset + '_result_sock_rca', 'rb+') as f:
        ...
        for i in range(len(result)):
            key = result[i][0]
            meth = result[i][1][0]
            data_rca = test_data[test_data['label'] == [v for k, v in dict_values.items() if k == key][0]]
            data = data_rca
            # print(data_rca)
            if result[i][1][0] == 'Lingam':
                for m in data.columns:
                    if m.split('_')[-1] != 'latency':
